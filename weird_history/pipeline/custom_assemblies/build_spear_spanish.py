...
def run():
    ...
    print("Generating voiceover for Spanish...")
    script_text = timeline.get("script", "")
    voice_id = timeline.get("tts_voice_id", "79a125e8-cd45-4c13-8a67-188112f4dd22")
    generate_tts(script_text, audio_path, voice_id, language="es")
    
    # 2. Generate transcriptions (using whisperx inline as in manual_assembly)
    print("Generating transcriptions for Spanish...")
    import whisperx
    device = "cpu" 
    ...
    # 3. Assemble Video
    print("Assembling Spanish video...")
    final_path = assemble_final_video(
        timeline, audio_path, words, "final_spear_video_spanish.mp4", output_dir=build_dir
    )
